...
def _street_groups(
    ...
    if street_order == "name":
        groups.sort(key=lambda item: str(item["street_key"]))
    ...
    if start_street_key is not None:
        groups.sort(key=lambda item: 0 if item["street_key"] == start_street_key else 1)
    return groups
...
def apply_segment_overrides(
    ...
    street_groups = {str(street_key): group for street_key, group in df.groupby("street_key", sort=False, dropna=False)}
    ...
    for unit in units:
        ...
        if requested_first_side != unit.first_side:
            group = street_groups.get(unit.street_key)
            if group is not None:
                street_rows, _ = _ordered_rows_for_street(group, requested_first_side, start_row_id)
        ...
        applied.append(
            (
                ...
                replace(unit, row_ids=row_ids, direction=direction, first_side=requested_first_side),
            )
        )
...
def build_map(ordered: pd.DataFrame):
    if folium is None:
            ...
            if len(unit_coords) >= 2:
                ...
                folium.PolyLine(
                    ...
                    tooltip=(
                        ...
                        f"{unit_id} | {first.get('Segment Direction', 'forward')}"
                    ),
                ).add_to(m)
